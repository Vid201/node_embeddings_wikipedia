...
for SEED in SEEDS:
    for window_size in [5, 10, 15]:
        for length in [40, 60, 80]:
            ...
            random_walks = random_walker.run(nodes=list(graph.nodes()), length=length, n=25, p=0.5, q=2.0, seed=SEED)
            ...
            albums_embeddings = pd.DataFrame({'node_id': node_ids}).merge(nodes, left_on='node_id',
                                                                          right_on='id').reindex(
                columns=['URL'])
            ...
            albums.to_csv(f'./embeddings/albums_embeddings_node2vec_BFS_{length}_{window_size}_{SEED}.csv',
                          index_label='id',
            ...
            print(
                f'seed: {SEED}, window size: {window_size}, length: {length}, accuracy: {accuracy}, time: {end_time - start_time}')
