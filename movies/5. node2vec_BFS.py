...
for SEED in SEEDS:
    for window_size in [5, 10, 15]:
        for length in [40, 60, 80]:
            ...
            movies_embeddings = pd.DataFrame({'node_id': node_ids}).merge(nodes, left_on='node_id',
                                                                          right_on='id').reindex(
                columns=['URL'])
            ...
            movies.to_csv(f'./embeddings/movies_embeddings_node2vec_BFS_{length}_{window_size}_{SEED}.csv',
                          index_label='id',
